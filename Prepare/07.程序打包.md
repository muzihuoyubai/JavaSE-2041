...
执行当前路径下`main.jar`中的`club.banyuan.machine.main.Main`这个类中的`main`方法

```
java -cp ./main.jar club.banyuan.machine.main.Main
```
...
Manifest-Version: 1.0
Created-By: 13-ea (Oracle Corporation)
club.banyuan.main.Main-Class: club.banyuan.machine.main.Main
```
...
```
jar cfe main.jar club.banyuan.machine.main.Main -C out .
```
...
```
java -cp out:lib/* club.banyuan.machine.main.Main
```

如果将`club.banyuan.machine.main.Main.class`单独打包出来为一个可执行jar，
...
```
java -cp lib/simulation.jar:main.jar club.banyuan.machine.main.Main
```
